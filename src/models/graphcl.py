"""GraphCL: Graph Contrastive Learning with NT-Xent loss."""
...
from src.config.schema import GraphCLConfig
...
class GraphCL(BaseSSLModel):
    """NT-Xent contrastive loss over two independently augmented views.

    Args:
        config: Config dict validated against GraphCLConfig.
        in_channels: Node feature dimensionality, resolved from the dataset.
    """

    def __init__(self, config: Dict, in_channels: int):
        super().__init__()
        cfg = GraphCLConfig.from_dict(config)
